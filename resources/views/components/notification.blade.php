<div
    ...
    @notify.window="let message = $event.detail[0]; messages.push(message); setTimeout(() => { remove(message) }, 7000)"
    ...
    <template x-for="(message, messageIndex) in messages" :key="messageIndex" hidden>
        ...
        >
            <div
                ...
                class="w-full max-w-sm overflow-hidden bg-noir-500 rounded-lg shadow-lg pointer-events-auto ring-2 ring-opacity-50 ring-black"
            >
                <div class="p-4">
                    <div class="flex items-start">
                        ...
                        <div class="ml-3 w-0 flex-1 pt-0.5">
                            <p x-text="message.title" class="font-medium text-white text-md"></p>
                            <p x-text="message.message" class="mt-1 text-sm text-white"></p>
                        </div>
                        <div class="flex flex-shrink-0 ml-4">
                            <button @click="remove(message)" class="inline-flex text-white bg-noir-900 rounded-full hover:bg-gray-500  transition duration-150 ease-in-out focus:outline-none">
